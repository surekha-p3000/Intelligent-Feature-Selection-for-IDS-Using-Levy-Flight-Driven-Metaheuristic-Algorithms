...
num_iterations = 50 # Increased number of iterations
...
# ----------------------------
# Run Aquila Optimization Algorithm (AOA) on Training Data
# ----------------------------
print("\n🚀 Running Aquila Optimization Algorithm...")

start_time_aoa = time.time()
best_features, best_score, optimization_convergence = aquila_optimization(
    X_train, y_train, dim,
    population_size, num_iterations, lb, ub,
    w1, w2, w3
)
end_time_aoa = time.time()
optimization_time = end_time_aoa - start_time_aoa

print("\n✅ Best AOA training fitness:", best_score)
print(f"AOA Optimization Time: {optimization_time:.2f} seconds")
...
print(f"\nSelected Features by AOA: {selected_feature_names}")

# Ensure selected_indices is not empty before proceeding
if selected_indices.size == 0:
    print("\n⚠️ No features selected by AOA. Cannot perform final evaluation.")
else:
    ...
    print("\n🎯 Final Test Metrics (with AOA Feature Selection):")
